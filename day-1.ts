...
3. check if the cookie portion s[firstIndex] is enough in children g[firstIndex]
4. if enough/satisfy the children greed, we pop/shift the children array & count the contentChildren
5. else we pop the cookie array
 */

function findContentChildren(g: number[], s: number[]): number {
    g.sort((min, max) => min - max);
    s.sort((min, max) => min - max);
